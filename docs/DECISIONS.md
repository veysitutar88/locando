...
- Implementation (Chunk #4 — actual): `src/shared/db/helpers.ts` exports `withTimestamps()`. All update paths in the repository layer pass payload through it. The type signature forbids passing `updatedAt` from the caller — the repository owns the field. Inserts use schema-level `defaultNow()` and don't need the helper. An ESLint `no-restricted-imports` rule (added in Chunk #4) blocks direct `@/shared/db/client` and `@/shared/db/schema` imports outside `src/shared/db/**` and `src/modules/**/lib/*repo.ts`, so missing tenant filtering is caught at lint time rather than runtime.
- Note on factory abstraction: a generic `tenantScoped<T>()` factory was considered for Chunk #4 but rejected. Drizzle generic typing required broad `as never` casts that would weaken security guarantees. For tenant isolation (a security boundary), explicit hand-written repositories are preferred over abstract factories. A factory may be revisited when 3–4 stable repos exist (likely Chunk #15+), once the pattern is mature enough to extract safely.
